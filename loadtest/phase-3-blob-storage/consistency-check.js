...
// request. Phase 3 flips Storage:Provider to AzureBlob, so every App
// Service instance reads and writes the same Blob container instead of its
// own local disk (Storage:Provider=Local, Phase 0-2). Now that uploads and
// downloads share a single backing store regardless of which instance
// serves which request, this script should pass cleanly even once App
// Service autoscales past one instance -- if it still fails here, that
// points at a Storage Account/Key Vault wiring problem, not architecture.
