#!/usr/bin/env bash
# Provisions Phase 2's App Service + Azure SQL Database + Key Vault + monitoring,
# then publishes and zip-deploys the app. Unlike the VM phases, App Service has
# no cloud-init -- Terraform only creates the empty Web App, so deploy.sh does
# the `dotnet publish` + `az webapp deploy` step itself.
set -euo pipefail
cd "$(dirname "$0")"
...
echo "Note: the ConnectionStrings__ClaimsDb app setting is a Key Vault reference -- if the" >&2
echo "access policy hadn't propagated by the app's first boot, restart it once with" >&2
